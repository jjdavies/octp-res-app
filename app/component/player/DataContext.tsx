...
import MatchAttempt from './interface/MatchAttempt';
...
export function DataProvider({ children }: DataProviderProps) {
  ...
  const [currentStageID, setCurrentStageID] = useState<string>('id001');
  ...
  const selectNewMatch = (resID: string) => {
    const resource = data?.stages
      ...
      .activitySettings.resources.filter((res) => res.resourceID === resID)[0];
    ...
    setMatchAttempts([...matchAttempts, { a: matchSelect, b: resID }]);
    return setMatchSelect('');
  };
  ...
  const navigateRight = () => {
    ...
    if (data) {
      ...
      setCurrentStageID(
        data.setup.stages[data.setup.stages.indexOf(currentStageID) + 1],
      );
    }
  };
  ...
  const triggerActions = (actions: Action[]) => {
    ...
    const animations = results.map((result) => {
      return {
        ...
        value: result.valueType === 'Actual Value' ? result.value.value : 0,
      };
    });
    ...
    //remove duplicates
    const uniqueRelevantResources = relevantResources.filter((resID, index) => {
      console.log(resID, index, relevantResources.indexOf(resID));
      return relevantResources.indexOf(resID) === index ? true : false;
    });
            ...
            return {
              ...
              activitySettings: {
                ...
                resources: stage.activitySettings.resources.map((res) => {
                  if (relevantResources.indexOf(res.resourceID) !== -1) {
                    return {
                      ...res,
                      resourceSettings: {
                        ...
                        animations: [
                          ...
                          ...animationObjects
                            .filter(
                              (animObj) =>
                                animObj.resourceID === res.resourceID,
                            )
